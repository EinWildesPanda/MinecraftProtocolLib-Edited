...
public class MinecraftProtocol extends PacketProtocol {
    ...
    public MinecraftProtocol(String username, String using, boolean token, boolean theAltening) throws RequestException {
        this(username, using, token,theAltening, Proxy.NO_PROXY);
    }

    public MinecraftProtocol(String username, String using, boolean token, boolean theAltening, Proxy authProxy) throws RequestException {
        this(SubProtocol.LOGIN);
        String clientToken = UUID.randomUUID().toString();
        AuthenticationService auth;
        if(!theAltening)
        {
            auth = new AuthenticationService(clientToken, authProxy);
        }
        else
            auth = new AuthenticationService(clientToken);

        auth.setUsingTheAltening(theAltening);
    ...
    public void newClientSession(Client client, Session session) {
        if(this.profile != null) {
            ...
            session.setFlag(MinecraftConstants.AUTH_PROXY_KEY, this.proxy);
        }
